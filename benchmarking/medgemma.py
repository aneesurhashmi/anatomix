...
def main(output_path, dataset_name, subset=None):
    model, processor = medgemma_init(device='cuda:0')
    ...
    for sample in tqdm(dataset, total=len(dataset)):
        if report_gen_task:
            ...
            prompt +=" Do not provide explanation."
        res = sample['text'][-1]['content']    

        out = medgemma_res([sample['images']], prompt, processor, model, max_new_tokens=200)
...
if __name__ == "__main__":
    ...
    filename = f"{args.dataset}/medgemma.csv"
